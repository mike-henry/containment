...
public class ApplRealm extends AuthorizingRealm
{
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken rawToken) throws AuthenticationException
	{
		...
		SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(token.getUsername(), token.getPassword(), token.getHash());
	...
	private String getPasswordFromUserName(String username)
	{
		...
		try{
			...
			Bean<UserRepository> userDAObean = (Bean<UserRepository>) beanManager.resolve(beanManager.getBeans(UserRepository.class));
	...
	private String doGetHashedPassword(String password, String challenge) throws NoSuchAlgorithmException, UnsupportedEncodingException
	{
